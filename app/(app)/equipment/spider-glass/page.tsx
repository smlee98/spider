...
type SortOption = "maxHeight-desc" | "maxHeight-asc" | "weight-desc" | "weight-asc";
...
export default function SpiderPage() {
  const [sortOption, setSortOption] = useState<SortOption>("maxHeight-desc");
  ...
  const EquipmentCard = ({ equipment }: { equipment: SpiderEquipment }) => (
    <Link
      ...
      href={`/equipment/spider-glass/${equipment.brandName.toLowerCase()}/${equipment.modelName.toLowerCase()}`}
      className="group"
    >
      <Card className="gap-0 overflow-hidden py-0 transition-all group-hover:ring-2">
        ...
        <CardContent className="bg-muted flex aspect-square items-center justify-center">
          <picture>
            <img
              src={`/equipment/spider-glass/${equipment.brandName}/${equipment.modelName}/${equipment.modelName}.png`}
  ...
  return (
    <Container>
      <div className="flex flex-col gap-6">
        <h1 className="text-foreground text-3xl font-black md:text-4xl">유리흡착기 거미크레인</h1>
